...
app.use(express.static("public_Usuario"));
...
app.get("/registro2",function (req,res) {
    ...
    conexion.query(
        ...
        `INSERT INTO tbl_usuarios (nombre, apellido, correo, nombre_usuario, contrasena, imagen_perfil, id_planes) VALUES (?,?,?,?,sha1(?),"img/usuarios/perfil.png",1)`,
...
app.get("/obtener-carpetas",function (req,res) {
    var conexion = mysql.createConnection(credenciales);
    var carpetas = [];
    conexion.query(
        `SELECT id_carpetas, nombre_carpeta, id_usuarios FROM tbl_carpetas WHERE id_usuarios=?`,
        [req.session.id_usuarios]).on("result",function(result){
...
app.get("/obtener-archivos",function(req,res) {
    var conexion = mysql.createConnection(credenciales);
    var carpetas = [];
    conexion.query(
        `SELECT id_archivos, nombre_archivo, id_lenguajes, id_carpetas, id_usuarios FROM tbl_archivos WHERE id_usuarios = ?`,
        [req.session.id_usuarios]).on("result",function (result) {
            carpetas.push(result);   
        })
        .on("end",function(){
